...
| 6 | Restart behavior | Add a first-class `agent-orchestrator-daemon restart [--force]` command, but do not auto-restart from the MCP frontend in this pass. | Restart is useful and discoverable, while automatic restart requires careful active-run policy and should not surprise users by orphaning or cancelling work. | Always auto-restart on mismatch; add only README workaround; make restart always forceful. |
| 7 | Documentation | Document the long-lived daemon upgrade behavior, mismatch error, `restart`, and `npx --package ... agent-orchestrator-daemon restart` recovery path. | The issue is operational; users need a clear remediation path even before automatic recovery exists. | Only update tests/code; only mention local `just orchestrator-restart`. |
...
| T5 | Add daemon restart CLI ergonomics | T1 | completed | `agent-orchestrator-daemon restart` stops then starts the daemon; default restart refuses active runs through existing stop semantics; `restart --force` force-stops first; help output in `src/cli.ts` and daemon CLI usage include restart; status output includes daemon version when available. |
